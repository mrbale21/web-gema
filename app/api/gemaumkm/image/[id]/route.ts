...
export async function GET(
  ...
  try {
    ...
    const GemaUmkmImgId = parseInt(id, 10);

    const GemaUmkmImg = await getGemaUmkmImgById(GemaUmkmImgId);
    if (!GemaUmkmImg) {
      return NextResponse.json(
        { error: "GemaUmkmImg not found" },
        { status: 404 }
      );
    }
...
export async function PUT(
  ...
  try {
    ...
    const updatedGemaUmkmImg = await updateGemaUmkmImg(GemaUmkmImgId, {
      ...data,
      image: imageUrl ?? data.image,
    });

    return NextResponse.json(updatedGemaUmkmImg);
  } catch (err: any) {
    return NextResponse.json({ error: err.message }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const deleted = await deleteGemaUmkmImg(id);
    return NextResponse.json({
      message: "GemaUmkmImg berhasil dihapus",
      deleted,
    });
  } catch (err: any) {
    return NextResponse.json({ error: err.message }, { status: 500 });
