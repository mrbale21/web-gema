...
export async function GET(
  ...
  try {
    ...
    const superiorId = parseInt(id, 10);

    const superior = await getSuperiorById(superiorId);
...
export async function PUT(
  ...
  try {
    ...
    const updatedSuperior = await updateSuperior(superiorId, {
      ...data,
      icon: iconUrl ?? data.icon,
    });

    return NextResponse.json(updatedSuperior);
  } catch (err: any) {
    return NextResponse.json({ error: err.message }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const deleted = await deleteSuperior(id);
