...
export default function ProductType({ product }: { product: Product }) {
  ...
  const fetchProductType = async () => {
    const res = await fetch(`/api/products/${product.id}/products-type`);
    const data = await res.json();
    setProductType(data);
  };

  useEffect(() => {
    if (product?.id) fetchProductType();
  }, [product?.id]);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `/api/products/${product.id}/products-type/${selectedProductType.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ title, desc, icon }),
  ...
  const handleDelete = async (id: number) => {
    setAlert({
      ...
      onConfirm: async () => {
        ...
        try {
          const res = await fetch(
            `/api/products/${product.id}/products-type/${id}`,
            {
              method: "DELETE",
            }
          );
  ...
  return (
    <div className=" text-gray-800">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Jenis {product.name}</h1>
        <button
