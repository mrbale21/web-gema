...
export default function GemaEdTechPage() {
  ...
  const [selected, setSelected] = useState<GemaEdTech | null>(null);
  ...
  const fetchData = async () => {
    const res1 = await fetch("/api/gemaedtech");
    setGemaEdTech(await res1.json());
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/gemaedtech", {
        method: "POST",
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/gemaedtech/${selected.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title, desc, title1, desc1 }),
      });
  ...
  const handleDelete = async (id: number) => {
    setAlert({
      ...
      onConfirm: async () => {
        setAlert(null);
        try {
          const res = await fetch(`/api/GemaEdTech/${id}`, {
            method: "DELETE",
          });
          const data = await res.json();
          if (!res.ok) {
