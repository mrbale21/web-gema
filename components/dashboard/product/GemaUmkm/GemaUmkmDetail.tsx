...
import { GemaUMKMDetail } from "@/types/gemaumkm";
...
export default function GemaUmkmDetailDashboard() {
  const [gemaUmkmDetail, setgemaUmkmDetail] = useState<GemaUMKMDetail[]>([]);
  const [addModal, setAddModal] = useState(false);
  const [editModal, setEditModal] = useState(false);
  const [error, setError] = useState("");
  const [selectedgemaUmkmDetail, setSelectedgemaUmkmDetail] =
    useState<GemaUMKMDetail | null>(null);
  ...
  const fetchgemaUmkmDetail = async () => {
    const res = await fetch("/api/gemaumkm/detail");
    const data = await res.json();
    setgemaUmkmDetail(data);
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedgemaUmkmDetail) return;

    try {
      const res = await fetch(
        `/api/gemaumkm/detail/${selectedgemaUmkmDetail.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ title, desc, icon }),
  ...
  const handleDelete = async (id: number) => {
    setAlert({
      ...
      onConfirm: async () => {
        setAlert(null);
        try {
          const res = await fetch(`/api/gemaumkm/detail/${id}`, {
            method: "DELETE",
          });
          const data = await res.json();
          if (!res.ok) {
  ...
  const resetForm = () => {
    ...
    setDesc("");
    setIcon("");
    setSelectedgemaUmkmDetail(null);
    setError("");
  };
  ...
  return (
    <div className="min-h-screen text-gray-800">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Keunggulan Gema Umkm</h1>
        <button
          ...
          >
            ...
            <div className="flex-1 flex flex-col gap-1">
              ...
              <div className="mt-2 flex gap-2 text-sm">
                <button
                  onClick={() => {
                    setSelectedgemaUmkmDetail(item);
                    setTitle(item.title);
                    setDesc(item.desc);
                    setIcon(item.icon || "");
                    setEditModal(true);
      ...
      {/* === MODAL UPDATE === */}
      {editModal && selectedgemaUmkmDetail && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
          <form
