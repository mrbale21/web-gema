...
export default function GemaUmkmImage() {
  ...
  const [selectedgemaUmkm, setSelectedgemaUmkm] = useState<GemaImgUMKM | null>(
    null
  );
  ...
  const fetchgemaUmkm = async () => {
    const res = await fetch("/api/gemaumkm/image");
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/gemaumkm/image/${selectedgemaUmkm.id}`, {
        method: "PUT",
        body: formData,
      });
  ...
  const handleDelete = async (id: number) => {
    setAlert({
      ...
      onConfirm: async () => {
        setAlert(null);
        try {
          const res = await fetch(`/api/gemaumkm/image/${id}`, {
            method: "DELETE",
          });
          const data = await res.json();
          if (!res.ok) {
