...
export default function GemaKopDetailDashboard() {
  ...
  const [selectedGemaKopDetail, setSelectedGemaKopDetail] =
    useState<GemaKopDetail | null>(null);
  ...
  const fetchGemaKopDetail = async () => {
    const res = await fetch("/api/gemakop/detail");
    const data = await res.json();
    setGemaKopDetail(data);
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedGemaKopDetail) return;

    try {
      const res = await fetch(
        `/api/gemakop/detail/${selectedGemaKopDetail.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ title, desc, icon }),
  ...
  const handleDelete = async (id: number) => {
    setAlert({
      ...
      onConfirm: async () => {
        setAlert(null);
        try {
          const res = await fetch(`/api/gemakop/detail/${id}`, {
            method: "DELETE",
          });
          const data = await res.json();
          if (!res.ok) {
  ...
  return (
    <div className="min-h-screen text-gray-800">
      ...
      {/* === MODAL UPDATE === */}
      {editModal && selectedGemaKopDetail && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
          <form
