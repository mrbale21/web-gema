...
export default function TrainingDetailDashboard() {
  ...
  const fetchtrainingDetail = async () => {
    const res = await fetch("/api/training/detail");
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedtrainingDetail) return;

    try {
      const res = await fetch(
        `/api/training/detail/${selectedtrainingDetail.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ title, desc, icon }),
  ...
  const handleDelete = async (id: number) => {
    setAlert({
      ...
      onConfirm: async () => {
        setAlert(null);
        try {
          const res = await fetch(`/api/training/detail/${id}`, {
            method: "DELETE",
          });
          const data = await res.json();
          if (!res.ok) {
  ...
  return (
    <div className=" text-gray-800">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Layanan Pelatihan</h1>
        <button
      ...
      {/* === MODAL UPDATE === */}
      {editModal && selectedtrainingDetail && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
          <form
