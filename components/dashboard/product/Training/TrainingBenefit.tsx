...
export default function TrainingBenefitPage() {
  ...
  const [selected, setSelected] = useState<TrainingBenefit | null>(null);

  const [title, setTitle] = useState("");

  // === FETCH DATA ===
  const fetchData = async () => {
    const res1 = await fetch("/api/training/benefit");
    settrainingBenefit(await res1.json());
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/training/benefit", {
        method: "POST",
      ...
      setAlert({
        type: "success",
        message: "Data trainingBenefit berhasil ditambahkan!",
      });
    } catch (err: any) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/training/benefit/${selected.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title }),
      });
      ...
      setAlert({
        type: "success",
        message: "Data trainingBenefit berhasil diperbarui!",
      });
    } catch (err: any) {
      setError(err.message);
    }
  ...
  const handleDelete = async (id: number) => {
    setAlert({
      ...
      onConfirm: async () => {
        setAlert(null);
        try {
          const res = await fetch(`/api/training/benefit/${id}`, {
            method: "DELETE",
          });
          const data = await res.json();
          if (!res.ok) {
  ...
  return (
    <div className=" text-gray-800">
      ...
      {/* === LIST trainingBenefit === */}
      <div className="space-y-6">
        {trainingBenefit.map((item) => (
          <div
            key={item.id}
